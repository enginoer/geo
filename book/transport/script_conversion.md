...
As the educational materials used a profile plot of concentration along the x-axis, this was added to the code. First, a few questions were asked to Copilot to ask it to restructure the functions and create a profile plot; the result had to be adjusted slightly (to get $R_d$ to be passed out of the concentration function).

The final code is shown here:

````{admonition} Python code with profile plot
:class: tip, dropdown

This is the Python code generated by GitHub Copilot, with a few "manual" modifications.
...
def plot_profile(M, ax, ay, Kp, lambda_, y_val=0):
    ...
    plt.title(f'Concentration Profile at Y = {y_val} m')
    plt.grid(True)
    plt.show()
